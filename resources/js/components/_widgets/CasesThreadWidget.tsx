...
import CaseThreadLoading from '@/components/case/loading/CaseThreadLoading';
import Button from '@/components/_elements/Button';
import SelectFilter from '@/components/filters/SelectFilter';
import { usePage } from '@inertiajs/react';
import { ApiResponse, Application, PageProps } from '@/types';
...
export default function CasesThreadWidget() {
  const { ziggy: query }: { ziggy: { query: QueryProps } } =
    usePage<PageProps>().props;

  const [response, setResponse] = useState<ApiResponse<Application>>();
  ...
  return (
    <>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        <Suspense fallback={<CaseThreadLoading />}>
          <CaseThread
            page={page}
            order={order}
            period={period}
